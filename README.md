...
🔑 Authentication
To access secured endpoints, you need a valid JWT token. Use the /auth/login endpoint to obtain a token:
...
🔑 Autenticação
Para acessar endpoints protegidos, você precisa de um token JWT válido. Use o endpoint /auth/login para obter um token:
